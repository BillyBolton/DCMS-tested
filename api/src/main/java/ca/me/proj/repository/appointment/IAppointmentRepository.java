...
public interface IAppointmentRepository extends CrudRepository<AppointmentEntity, Long> {
    ...
    @Query(value = "SELECT CASE WHEN count(a)> 0 THEN true ELSE false end from APPOINTMENT a where ((?1 <= a.end_time AND ?1 >= a.start_time) OR (?2 <= a.end_time AND ?2 >= a.start_time)) AND patient_id = ?3", nativeQuery = true)
    public boolean findPatientScheduleConflict (Date start_time, Date end_time, String id);

    @Query(value = "SELECT CASE WHEN count(a)> 0 THEN true ELSE false end from APPOINTMENT a where ((?1 <= a.end_time AND ?1 >= a.start_time) OR (?2 <= a.end_time AND ?2 >= a.start_time)) AND employee_id = ?3", nativeQuery = true)
